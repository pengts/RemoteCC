<script lang="ts">
  ...
  let sidebarWidth = $state(
    typeof window !== "undefined"
      ? Number(localStorage.getItem("ocv:sidebar-width")) || SIDEBAR_DEFAULT
      : SIDEBAR_DEFAULT,
  );
  ...
  onMount(() => {
    ...
    // Team/task events come via WebSocket → CustomEvent dispatch
    const handleTeamUpdate = (e: Event) => {
      const payload = (e as CustomEvent).detail as TeamUpdatePayload;
      dbg("layout", "team-update", payload);
      teamStore.handleTeamUpdate(payload);
    };
    const handleTaskUpdate = (e: Event) => {
      const payload = (e as CustomEvent).detail as TaskUpdatePayload;
      dbg("layout", "task-update", payload);
      teamStore.handleTaskUpdate(payload);
    };
    window.addEventListener("ocv:team-update", handleTeamUpdate);
    window.addEventListener("ocv:task-update", handleTaskUpdate);
    scheduleResync();
    ...
    return () => {
      ...
      if (debounceTimer) clearTimeout(debounceTimer);
      destroyed = true;
      window.removeEventListener("ocv:team-update", handleTeamUpdate);
      window.removeEventListener("ocv:task-update", handleTaskUpdate);
  ...
  async function handleDeleteRuns(runIds: string[]) {
    try {
      await Promise.all(runIds.map((id) => deleteRun(id)));
      window.dispatchEvent(new Event("ocv:runs-changed"));
    } catch (e) {
      dbgWarn("layout", "delete runs failed", e);
    }
  }
  ...
  async function pickFolder() {
    try {
      const selected = prompt(t("layout_selectProjectFolder"), projectCwd || "/");
      if (selected) projectCwd = normalizeCwd(selected) || "";
    } catch (e) {
...
<div class="flex h-screen overflow-hidden" class:select-none={resizing}>
  ...
  {#if sidebarOpen}
    <aside class="flex shrink-0 bg-sidebar text-sidebar-foreground transition-all duration-200">
      ...
      <!-- B. Content Panel -->
      <div
        class="flex flex-col overflow-hidden border-r border-sidebar-border"
    ...
    <!-- svelte-ignore a11y_no_static_element_interactions -->
    <div
      class="w-1.5 shrink-0 self-stretch cursor-col-resize hover:bg-primary/30 active:bg-primary/50 transition-colors {resizing ? 'bg-primary/50' : ''}"
