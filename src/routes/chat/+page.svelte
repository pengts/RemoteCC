<script lang="ts">
  ...
  onMount(() => {
    ...
    // Screenshot: not available in web mode (no global hotkey capture)

    // HTML5 drag-drop listeners (replacing Tauri native drag-drop)
    ...
    function onDrop(e: DragEvent) {
      ...
      if (files?.length && promptRef) {
        promptRef.addFiles(Array.from(files));
      }
    }
  ...
  // Listen for auto-context snapshots from Rust backend (via WebSocket → CustomEvent)
  onMount(() => {
    const handler = (e: Event) => {
      const { runId, content, turnIndex, ts } = (e as CustomEvent).detail as {
        runId: string;
        content: string;
        turnIndex: number;
        ts: string;
    ...
    window.addEventListener("ocv:context-snapshot", handler);
    return () => {
      window.removeEventListener("ocv:context-snapshot", handler);
    };
  });
  ...
  async function sendMessage(text: string, attachments: Attachment[]) {
    ...
    try {
      if (!store.run) {
        ...
        if (!cwd || cwd === "/") {
          const selected = prompt(t("layout_selectProjectFolder"), "/");
