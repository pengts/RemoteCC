...
export function buildProjectFolders(
  ...
  for (const [cwd, bucketRuns] of cwdBuckets) {
    ...
    for (const [sessionId, sessionRuns] of sessionMap) {
      ...
      const title = latestRun.name?.trim() || latestRun.cli_slug?.trim() || earliestRun.prompt?.trim() || "Untitled";
    ...
    for (const run of standalone) {
      const title = run.name?.trim() || run.cli_slug?.trim() || run.prompt?.trim() || "Untitled";
      conversations.push({
        groupKey: `r:${run.id}`,
        runs: [run],
        title,
