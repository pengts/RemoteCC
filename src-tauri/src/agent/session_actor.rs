...
impl SessionActor {
    ...
    /// Send an event via WebSocket broadcast channel.
    fn ws_emit<T: serde::Serialize>(&self, event_name: &str, payload: &T) {
        match serde_json::to_value(payload) {
            Ok(v) => {
                let msg = serde_json::json!({ "event": event_name, "payload": v });
                let _ = self.ws_tx.send(msg.to_string());
            }
            Err(e) => log::warn!("[actor] ws_emit serialize failed: {}", e),
        }
    }
    ...
    async fn handle_control_event(&mut self, parsed: &Value, event_type: &str) {
        ...
        if subtype == "hook_callback" {
            ...
            self.ws_emit("bus-event", &BusEvent::Raw {
                    run_id: self.run_id.clone(),
