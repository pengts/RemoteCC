...
pub struct SyncResult {
    ...
    pub usage_incomplete: bool,
    /// True when RunMeta was modified (e.g. cli_slug changed) even if new_events == 0.
...
pub fn import_session(
    ...
    for line_result in reader.lines() {
        ...
        if let Ok(json_val) = serde_json::from_str::<Value>(trimmed) {
            ...
            // Extract latest slug (CLI session name)
            if let Some(slug) = json_val.get("customTitle").and_then(|v| v.as_str()) {
...
pub fn sync_session(
    ...
    if current_size == watermark.offset {
        // No new data — but rescan slug if missing OR if mtime changed (CLI may have renamed)
        let mtime_changed = current_mtime_ns != watermark.mtime_ns;
        let need_slug_scan = meta.cli_slug.is_none() || mtime_changed;
        let mut meta_updated = false;

        if need_slug_scan {
            if let Ok(content) = fs::read_to_string(&cli_path) {
                let mut scanned_slug: Option<String> = None;
                for line in content.lines() {
                    let trimmed = line.trim();
                    if trimmed.is_empty() {
                        continue;
                    ...
                    if let Ok(json_val) = serde_json::from_str::<Value>(trimmed) {
                        if let Some(slug) = json_val.get("customTitle").and_then(|v| v.as_str()) {
                            scanned_slug = Some(slug.to_string());
                        }
                    }
                }
                if scanned_slug.is_some() && scanned_slug != meta.cli_slug {
                    let mut updated = meta.clone();
                    updated.cli_slug = scanned_slug;
                    let _ = super::runs::save_meta(&updated);
                    meta_updated = true;
                    log::debug!("[cli_sessions] sync: updated cli_slug for run {}", run_id);
                }
            }
        }
    ...
    while byte_offset < watermark.offset {
        ...
        if let Ok(json_val) = serde_json::from_str::<Value>(trimmed) {
            // Track slug from warmup lines for backfill
            if let Some(slug) = json_val.get("customTitle").and_then(|v| v.as_str()) {
                last_slug = Some(slug.to_string());
            }
    ...
    let old_slug = updated_meta.cli_slug.clone();
    ...
    // Sync CLI slug (session name) — only update cli_slug, never touch user-set name
    if let Some(slug) = last_slug {
        updated_meta.cli_slug = Some(slug);
    }
    let slug_changed = updated_meta.cli_slug != old_slug;
