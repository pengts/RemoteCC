...
pub fn delete_run(id: &str) -> Result<(), String> {
    log::debug!("[storage/runs] delete_run: id={}", id);
    let dir = super::run_dir(id);
    if !dir.exists() {
        return Err(format!("Run {} not found", id));
    }
    // Delete the source CLI session file so auto-sync won't re-import
    if let Some(meta) = get_run(id) {
        if let Some(ref cli_path) = meta.cli_session_path {
            let p = std::path::Path::new(cli_path);
            if p.exists() {
                if let Err(e) = fs::remove_file(p) {
                    log::warn!("[storage/runs] delete_run: failed to remove CLI file {:?}: {}", p, e);
                }
            }
        }
    }
    fs::remove_dir_all(&dir).map_err(|e| format!("Failed to delete run {}: {}", id, e))
}
