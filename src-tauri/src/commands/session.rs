...
fn emit_bus_event(state: &AppState, writer: &EventWriter, run_id: &str, event: &BusEvent) {
    if let Err(e) = writer.write_bus_event(run_id, event) {
        log::warn!("[session] persist failed: {}", e);
    }
    state.emit_ser("bus-event", event);
}
...
pub async fn start_session(
    ...
    let _guard = state.spawn_locks.acquire(&run_id).await;
    ...
    emit_bus_event(&state, &state.event_writer, &run_id, &spawning_event);
    storage::runs::update_status(&run_id, RunStatus::Running, None, None).ok();

    // 5. Stop any existing actor for this run_id
    let had_session = stop_actor(&state.actor_sessions, &run_id).await?;
    if had_session {
        log::debug!(
    ...
    let actor_handle = session_actor::spawn_actor(
        state.ws_tx.clone(),
        Arc::clone(&state.event_writer),
        state.actor_sessions.clone(),
        run_id.clone(),
        child,
        stdin,
        stdout,
    ...
    state.actor_sessions.lock().await.insert(run_id.clone(), actor_handle);
    ...
    if let Some(text) = initial_text {
        ...
        emit_bus_event(&state, &state.event_writer, &run_id, &idle_event);
        log::debug!(
...
pub async fn send_session_message(
    ...
    // Get channel sender
    let cmd_tx = {
        let map = state.actor_sessions.lock().await;
        map.get(&run_id)
...
pub async fn stop_session(
    ...
    let was_active = stop_actor(&state.actor_sessions, &run_id).await?;
    if was_active {
        ...
        emit_bus_event(&state, &state.event_writer, &run_id, &event);
...
pub async fn approve_session_tool(
    ...
    stop_actor(&state.actor_sessions, &run_id).await?;
